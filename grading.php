		...
		if($type==="Function"){
			...
			if($score>0){
				...
				for($i=0; $i<strlen($str); $i++){
					...
					if($flag && ctype_alpha($str[$i])){
						array_push($params,$str[$i]);
					}
				}
				$emptyParams=false;
				...
				$returnPattern="/( +|\t+)+return [a-zA-Z]([\+\-\*\/]([a-zA-Z]|[0-9]+))?/";
				if(sizeof($params)==2){
					$equationPattern="/( +|\t+)+".$returned."=".$params[0]."[\+\-\*\/]".$params[1]."([\+\-\*\/][0-9]+)*/";
					$equationPattern2="/( +|\t+)+".$returned."=".$params[1]."[\+\-\*\/]".$params[0]."([\+\-\*\/][0-9]+)*/";
					...
					$equationPattern="/( +|\t+)+".$returned."=".$params[0]."[\+\-\*\/][0-9]+/";
				...
				for($i=0; $i<sizeof($testCases)-1; $i++){
					$line5=$line5.$character."=sys.argv[".($i+1)."]\n";
				...
				for($i=0; $i<sizeof($testCases)-1; $i++){
					if($i<sizeof($testCases)-2){
						$first=null;
						if(is_numeric($testCases[$i])){
							$first="int(".$characters[$i]."),";
						}
						else{
							$first=$characters[$i].",";
						}
				...
				for($i=0; $i<sizeof($testCases); $i++){
					$execute=$execute.' '.$testCases[$i];
				}
				$command=escapeshellcmd($execute);//executing the file with test cases
			...
			if($score>0){
				...
				if(sizeof($params)===2){
					...
					$equationPattern="/( +|\t+)+".$returned."=".$params[0]."\*\*[".$params[0]."0-9]/";
					if(!preg_match($equationPattern,$lines[sizeof($lines)-2])){
						$score-=$deduct;
						$comment=$comment."-".$deduct.": Arithmetic statement is not formatted properly.\n";
					}
					...
					$equationPattern="/( +|\t+)+".$returned."=[a-zA-Z0-9][\*\*[a-zA-Z0-9]]+/";
					if(!preg_match($equationPattern,$answer)){
						$score-=$deduct;
						$comment=$comment."-".$deduct.": Arithmetic statement is not formatted properly.\n";
					}
				}
				...
				if(sizeof($params)==2){
					...
					for($i=0; $i<sizeof($params); $i+=2){
						if($i<sizeof($params)-2){
							$line7=$line7."int(".$params[$i]."),int(".$params[$i+1]."),";
						}
						else{
							$line7=$line7."int(".$params[$i]."),int(".$params[$i+1]."))\n";
						}
				...
				if(is_null($output)){
					...
					if(sizeof($testCases)<=3){
						...
						$correct=false;
						for($i=1; $i<sizeof($testCases); $i++){
							if($output===$testCases[$i]){
								$correct=true;
							}
						}
						if(!$correct){
